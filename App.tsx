...
import { SettingsIcon, NewIcon, DeleteIcon } from './components/icons'; 
...
const DEFAULT_PROJECT: Project = {
  id: 'proj-001', name: '', clientAddress: '', city: '', postalCode: '', altitude: 0, windZone: WindZone.ZONE_1, distanceToPanel: 10,
...
function App() {
  ...
  useEffect(() => {
    ...
    async function checkAndRefresh() {
      ...
      if (savedBuildId !== buildId) {
        console.log(`Nouvelle version dÃ©tectÃ©e (). RafraÃ®chissement...`);
        localStorage.setItem(VERSION_STORAGE_KEY, buildId);
  ...
  useEffect(() => {
    ...
    if (totalPowerW > 0) {
        ...
        if (project.inverterConfig.brand === InverterBrand.FOXESS) {
            const liycyComp = cableDB['CAB14124172'] || { id: 'CAB14124172', description: 'CABLE LIYCY 2X0.75 C50M', unit: 'piece', price: 'A4SCW0' };
        ...
        if (((project.inverterConfig.brand === InverterBrand.FOXESS && !project.inverterConfig.model?.includes('MICRO')) || project.inverterConfig.brand === InverterBrand.CUSTOM) && !isMicroSystem) {
             ...
             project.fields.forEach(field => {
                 const fDc = calculateDcCableSection(getPanelCount(field.panels), project.distanceToPanel, cableDB as any);
                 if (fDc) { 
                     totalDc += fDc.quantity; 
                     if (!dcComp) dcComp = fDc; 
                 }
             });
             
             if (dcComp) {
                 if (dcComp.id === '821101000609200') {
                     totalDc = Math.max(1, Math.ceil(totalDc));
                 }
                 globalBOM.push({ ...dcComp, quantity: totalDc });
             }

             const connFem = inverterDB['32.0316P0010-UR'] || { id: '32.0316P0010-UR', description: 'CONNECTEUR FEM.MC4 EVO2-10 piÃ¨ces', unit: 'piece', price: 'A0C085' };
             ...
             const connMale = inverterDB['32.0317P0010-UR'] || { id: '32.0317P0010-UR', description: 'CONNECTEUR MALE MC4 EVO2-10 piÃ¨ces', unit: 'piece', price: 'A0C077' };
        ...
        if (activeInverter) {
            ...
            systemTotalAcPowerVA = totalPowerW; // Fallback DC si pas d'onduleur sÃ©lectionnÃ©
        }
        ...
        // Calibre requis = I * 1.25 (Coefficient sÃ©curitÃ© PV continu)
        ...
        // --- SÃLECTION COFFRET AC ---
        if (project.inverterConfig.brand === InverterBrand.ENPHASE) {
            if (isThreePhase) acBoxId = '13488'; // 16A Tri (Supporte ~11kW)
            else {
                // Si l'utilisateur a configurÃ© plusieurs branches, on privilÃ©gie les coffrets adaptÃ©s (1/2/3 Q-Relay)
            ...
            if (isThreePhase) acBoxId = '13498'; // 16A Tri
                ...
                // Coffrets multi-strings: 1 / 2 / 3 dÃ©parts (selon nb de branches)
            ...
            if (hasBackup) {
                ...
                if (isThreePhase) acBoxId = requiredBreakerRating <= 16 ? '13474' : '13476';
                    ...
                    else acBoxId = '13446'; // Fallback 63A si puissance > 9kW (utilise rÃ©f APS 63A standard)
                }
            }
            
            // --- SÃLECTION COFFRET DC ---
        ...
        // --- CALCUL DISJONCTEUR SI AGCP RENSEIGNÃ ---
    ...
    setMaterials(finalizedBOM);
  }, [project.fields, project.system, project.inverterConfig, project.evCharger, project.distanceToPanel, k2DB, esdecDB, inverterDB, boxDB, cableDB]);
  ...
  const acCableMaterial = materials.find(m => m.description.includes('CABLE R2V') && (m.description.includes('3G') || m.description.includes('5G')));
  let acCableSection = 2.5;
  if (acCableMaterial) {
      const desc = acCableMaterial.description;
      const match = desc.match(/[35]G(\d+\.?\d*)/);
  ...
  const compatibilityReport = useMemo(() => {
          ...
          if (requiresStringConfig) {
              ...
              if (totalAssigned !== totalPanelsCount) {
                  return {
                      isCompatible: false,
                      warnings: [],
                      errors: [`RÃ©partition Incorrecte : ${totalAssigned} panneaux assignÃ©s sur ${totalPanelsCount} disponibles.`],
                      details: null
                  };
              }
          }
      ...
      return null;
  }, [project.fields, project.inverterConfig.configuredStrings, project.inverterConfig.stringsCount, project.inverterConfig.model, projectClimate, activeInverterComp, inverterDB]);
      ...
      <header className="bg-indigo-900 text-white p-4 shadow-lg sticky top-0 z-50">
        <div className="container mx-auto flex justify-between items-center">
            <h1 className="text-2xl font-bold flex items-center gap-2">
                <span className="text-orange-500 text-3xl">â </span> Richardson Solaire <span className="text-xs bg-green-500 px-2 py-1 rounded-full">v3.0</span>
            </h1>
            ...
            {!isProjectStep && (
                <div className="hidden md:flex items-center gap-6 bg-indigo-800/50 px-4 py-1.5 rounded-full border border-indigo-700 animate-scale-in">
                    ...
                    <div className="flex flex-col border-l border-indigo-700 pl-4">
                        ...
                        <span className="text-xs font-black">{project.postalCode} â¢ {project.city || 'CP saisi'}</span>
                    </div>
            ...
            <div className="flex gap-4 items-center">
                 {user ? (
                     <div className="flex items-center gap-2">
                         ...
                         <button onClick={() => setView('admin')} className="p-2 bg-indigo-800 rounded-lg hover:bg-orange-500 transition-colors shadow-inner" title="GÃ©rer la base de donnÃ©es"><SettingsIcon className="w-5 h-5"/></button>
                         <button onClick={handleLogout} className="text-xs text-indigo-300 hover:text-white px-3 py-1 border border-indigo-700 rounded-md">DÃ©connexion</button>
                     </div>
                 ) : (
      ...
      <main className="container mx-auto p-4 relative">
                ...
                <div className="bg-white p-8 rounded-2xl shadow-2xl border-t-8 border-blue-600 max-w-2xl w-full">
                    <div className="flex items-center gap-4 mb-6">
                        ...
                        <div>
                            ...
                            <p className="text-sm text-slate-500 font-medium">Saisissez les informations clients pour dÃ©bloquer les calculs normatifs.</p>
                        </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                        <div className="space-y-4">
                            <div>
                                ...
                                <input type="text" value={project.name} onChange={(e) => setProject(p => ({...p, name: e.target.value}))} className="w-full p-3 border rounded-xl text-sm focus:ring-4 focus:ring-blue-100 outline-none font-bold border-slate-200" placeholder="Ex: M. Jean DUPONT"/>
                            </div>
                            <div>
                        ...
                        <div className="space-y-4">
                            ...
                            <div>
                                ...
                                <input type="number" value={project.altitude} onChange={(e) => setProject(p => ({...p, altitude: parseInt(e.target.value) || 0}))} className="w-full p-3 border rounded-xl text-sm font-black text-center focus:ring-4 focus:ring-blue-100 outline-none border-slate-200" placeholder="0"/>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="flex flex-col items-center gap-4">
                        ...
                        <button onClick={() => setShowWindGuide(true)} className="text-blue-600 font-bold text-xs hover:underline uppercase tracking-widest">Consulter la carte des tempÃ©ratures basses</button>
                    </div>
                </div>
            </div>
        )}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
            <div className="lg:col-span-3 space-y-4">
                ...
                {/* 2. GROUPE TOITURE + PANNEAUX (INDENTÃ) */}
                <div className="space-y-4">
                        ...
                        <div className="flex gap-1 overflow-x-auto mb-3">
                            {project.fields.map((f, i) => (
                                <button key={f.id} onClick={() => setActiveFieldIndex(i)} className={`px-2 py-1 text-[10px] rounded-t font-black uppercase transition-all ${activeFieldIndex === i ? 'bg-orange-5 text-orange-600 border-b-2 border-orange-500' : 'text-slate-400'}`}>
                                    {f.name} {i > 0 && <span onClick={(e) => { e.stopPropagation(); setProject(p => ({...p, fields: p.fields.filter((_, idx) => idx !== i)})); setActiveFieldIndex(0); }} className="ml-1 text-red-400">Ã</span>}
                                </button>
                            ))}
                        </div>
                        <div className="space-y-4">
                            ...
                            <div className="bg-orange-50/50 rounded-lg border border-orange-100 overflow-hidden">
                                <button onClick={() => setShowMargins(!showMargins)} className="w-full flex justify-between items-center p-3 hover:bg-orange-100/50 transition-all group">
                                    <span className="text-[9px] font-black text-orange-600 uppercase tracking-widest">Marges de sÃ©curitÃ© (mm)</span>
                        ...
                        <div className="flex items-center gap-2 mb-3">
                            <select value={activeField.panels.model.name} onChange={(e) => {
                                ...
                                {(Object.values(panelDB) as SolarComponent[]).sort((a: SolarComponent, b: SolarComponent) => {
                                    if (a.id === 'Panneau PersonnalisÃ©') return 1;
                                    if (b.id === 'Panneau PersonnalisÃ©') return -1;
                ...
                <div className="bg-white p-5 rounded-xl shadow-md border-t-4 border-purple-500">
                    <div className="flex justify-between items-center mb-4">
                        <h3 className="font-black text-slate-700 uppercase tracking-wider text-xs">Ãlectrique</h3>
                    ...
                    <div className="bg-slate-50 p-3 rounded-xl border border-slate-100 mb-4 flex items-center justify-between">
                        ...
                        <div className="flex flex-col items-end gap-1">
                            <Tooltip content="Puissance du disjoncteur d'abonnÃ© (AGCP) pour dÃ©terminer la protection AC" position="left">
                    ...
                    <div className="flex items-center gap-2 mb-2">
                        <select value={project.inverterConfig.brand} onChange={(e) => {
                            ...
                            <option value="Custom">Onduleur PersonnalisÃ©</option>
                        </select>
                        {isCustomInverter && (
                    ...
                    {(project.inverterConfig.brand !== 'None' && project.inverterConfig.brand !== 'Custom') && (
                        <div className="space-y-3">
                            <select value={project.inverterConfig.model} onChange={(e) => {
                                ...
                                <option value="">-- SÃ©lectionner ModÃ¨le --</option>
                                {(Object.values(inverterDB) as SolarComponent[]).filter(c => {
                                    ...
                                    const isAccessory = /\b(cable|cÃ¢ble|cap|relay|term|conn|bouchon|terminaison|relais|rallonge|envoy|ct-100|SMG666|DDSU|DTSU|bus|q-cable|AC-7\.0|AC-22\.0)\b/i.test(c.id) || /\b(cÃ¢ble|cable|bouchon|terminaison|relais|connecteur|passerelle|transformateur|compteur|embout|bus|borne|recharge)\b/i.test(c.description.toLowerCase());
                    ...
                    {project.inverterConfig.brand !== 'None' && (
                        <div className="mt-3 space-y-3">
                            {((project.inverterConfig.brand === 'FoxESS' && !project.inverterConfig.model?.includes('MICRO')) || isCustomInverter) && project.inverterConfig.model && !isMicroSystem && (
                                <div className="bg-white p-3 rounded-lg border border-slate-200 shadow-sm animate-scale-in">
                                    <div className="flex justify-between items-center mb-2 pb-2 border-b border-slate-100">
                                        <label className="text-[9px] font-black text-slate-500 uppercase tracking-widest">Configuration des ChaÃ®nes DC</label>
                                        <div className="flex items-center gap-2">
                                            <div className={`px-2 py-0.5 rounded border text-[10px] font-black flex items-center gap-1 ${assignmentComplete ? 'bg-green-100 text-green-700 border-green-200' : 'bg-orange-50 text-orange-600 border-orange-200'}`}>
                                                ...
                                                {assignmentComplete ? <span>â</span> : <span>â ï¸</span>}
                                            </div>
                                            ...
                                            return (
                                                <div key={currentMppt} className="bg-slate-50 border border-slate-200 rounded-lg p-2">
                                                    <div className="flex justify-between items-center mb-2">
                                                        ...
                                                        <span className="text-[9px] font-bold text-slate-500">{panelsOnMppt} Panneaux Total</span>
                                                    </div>
                            ...
                            <div className="bg-slate-50 p-2 rounded-lg border border-slate-200">
                                <label className="block text-[9px] font-black text-slate-500 uppercase tracking-widest mb-1">CÃ¢blage (m)</label>
                        ...
                        <div className="space-y-3 animate-scale-in">
                            ...
                            <div className="pt-2 border-t border-slate-100">
                                <label className="flex items-center gap-2 cursor-pointer group">
                                    ...
                                    />
                                    <span className="text-xs font-bold text-slate-600 group-hover:text-green-700 transition-colors">Ajouter le cÃ¢ble de charge</span>
                                </label>
                                    ...
                                    <div className="mt-2 bg-slate-50 border border-slate-200 rounded-lg p-2 animate-scale-in">
                                        {project.evCharger.phase === 'Tri' ? (
                                            <div className="text-[10px] font-bold text-green-700 text-center uppercase tracking-tighter">
                                                CÃ¢ble T2 22kW (Inclus par dÃ©faut en Tri)
                                            </div>
                                        ) : (
                                            <div className="flex gap-1">
                                                <button 
                                                ...
                                                >
                                                    CÃ¢ble polyvalent (Mono jusqu'Ã  7kW et TriphasÃ© jusqu'Ã  22kW)
                                                </button>
                                            </div>
                                        )}
                                    </div>
