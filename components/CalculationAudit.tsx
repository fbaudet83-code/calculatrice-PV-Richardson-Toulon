      ...
      <div className="p-6 space-y-8">
            ...
            <div className="mt-4 p-4 bg-slate-800 rounded-xl text-white shadow-inner">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3">
                   {stringsAnalysis.map((mppt, idx) => {
                       ...
                       return (
                           <div key={idx} className={`p-3 rounded-lg border relative overflow-hidden transition-colors ${isLowVmp ? 'bg-red-900/30 border-red-500/50' : 'bg-slate-700/50 border-slate-600/50'}`}>
                              <div className="flex justify-between items-start mb-2">
                                 ...
                                 <span className={`text-[11px] font-black px-1.5 py-0.5 rounded ${isLowVmp ? 'bg-red-500 text-white' : 'bg-blue-400/10 text-blue-400'}`}>
                              ...
                              <div className="space-y-1">
                                  ...
                                  <div className="flex justify-between items-baseline border-t border-slate-600/50 pt-1">
                                     ...
                                     <span className={`text-[10px] font-mono font-bold ${isLowVmp ? 'text-red-500 animate-pulse font-black' : 'text-green-300'}`}>
                ...
                {lowVoltageErrors.length > 0 && (
                    <div className="mt-4 p-4 bg-red-600/20 border-2 border-red-500 rounded-lg animate-pulse">
                        <div className="flex items-start gap-3">
                            <span className="text-2xl">⚠️</span>
                            <div>
                                <h4 className="text-red-400 font-black uppercase text-xs mb-1">Attention : Configuration Incompatible</h4>
                                {lowVoltageErrors.map((err, i) => (
                                    <p key={i} className="text-[11px] font-bold text-white leading-tight mb-1">
                                        MPPT #{err.mpptIndex} : valeur VMP à chaud ({err.vmpHot.toFixed(1)}V) Non conforme.
                                        <br/>
                                        <span className="text-red-200 font-normal italic">Se reporter à la notice constructeur (Min MPPT: {details.vminMppt}V).</span>
                                    </p>
                                ))}
        ...
        <section>
          <div className="flex justify-between items-start mb-3">
            ...
            <span className={`px-2 py-1 rounded text-[10px] font-bold ${voltageDrop <= 1 ? 'bg-green-100 text-green-700' : 'bg-orange-100 text-orange-700'}`}>
              {voltageDrop <= 1 ? 'DU < 1% OK' : 'DU > 1% ALERTE'}
            </span>
          </div>
            ...
            <div className="bg-white rounded-xl shadow-2xl w-full max-w-xl max-h-[80vh] flex flex-col overflow-hidden relative border border-slate-200">
                ...
                <div className="p-4 space-y-4 text-sm text-slate-700 overflow-y-auto flex-1 bg-slate-50/30">
                    ...
                    <div className="pb-1">
                        ...
                        <div className="mt-1 text-[10px] flex items-center gap-2 justify-end">
                            <span className="font-bold text-slate-500">Plage MPPT Min :</span> 
                            ...
                            {details.vmpHot < details.vminMppt ? "❌ NON CONFORME" : "✅ OK"}
                        </div>
                    </div>
